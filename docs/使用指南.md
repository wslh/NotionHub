...
### 第 1 步：复制 Notion 模板
打开 [WeRead2Notion 模板页](https://app.notion.com/p/wph/Template-3a329affe5af800b8581f98b71e948fb)，点击右上角 **Duplicate**，复制到你自己的 Workspace。
...
2. 名称填 `WeRead2Notion-AI`，Workspace 选模板所在的 Workspace。
...
5. 回到你的 Notion 模板页面，右上角 `••• → Connections`，把 `WeRead2Notion-AI` 加进去。
...
- 页面 `••• → Connections` 是否加了 `WeRead2Notion-AI`？
- `NOTION_TOKEN` 是否属于该 Integration、且在同一 Workspace？

**日志里显示的 Integration 名称不对**
- 名称由 `NOTION_TOKEN` 决定；把 Token 换成你自己创建的 `WeRead2Notion-AI` Secret。
