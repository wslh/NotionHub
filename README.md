...
2. **账号**：Notion / GitHub OAuth 登录入口。**独立发布版（wslh/NotionHub）已移除上游内置的 OAuth 客户端凭据**——默认不再携带任何 Client ID / Client Secret，也不会再调用上游托管的 token-exchange 代理（`notion-auth.notionhub.app`）。因此首次使用需先在面板高级设置里填写**你自己的** Notion Public Integration（或 GitHub OAuth App）的 Client ID + Client Secret（详见下方「独立发布版如何配置自己的 OAuth」）；填写后点「登录账号」会直连 `api.notion.com/v1/oauth/token` 用你的凭据换 `access_token`，并在你的 Public Integration 配置了「模板页 URL」且授权时选了「复制模板」的情况下自动复制模板。结果存到本机 `chrome.storage.local`。未配置时会提示「未配置 Client ID」。GitHub 卡片为外链入口。

### 独立发布版如何配置自己的 OAuth

> ⚠️ **重要**：本仓库（wslh/NotionHub）是**独立发布版**，已从 `background.js` 中移除上游项目的 Notion / GitHub OAuth 客户端默认值，也不再依赖上游托管的 token-exchange 代理（`NOTIONHUB_TOKEN_EXCHANGE_URL` 已置空）。因此控制面板「账号」标签页的「登录账号 / GitHub 绑定」按钮**不会**再走上游公共 OAuth 应用或第三方代理。要让「一键授权 / 自动复制模板」可用，你必须**自建一套 OAuth 应用**并在面板高级设置里填写凭据；否则按钮会提示「未配置 Client ID」。

**为什么必须自建（而不是沿用上游默认值）：**

- 上游内置的 Client ID 属于原作者（`hodgekou`）的 Notion / GitHub Integration。若沿用，会把你的授权流量导向**原作者**的 OAuth 应用与代理，违背独立发布与最小权限原则。
- 上游的 token-exchange 代理（`notion-auth.notionhub.app`）代持 OAuth Client Secret 并负责换 token，独立发布版不应再依赖任何第三方代理。
- 早期提交已把硬编码的 Client Secret 置空（安全红线）；保留上游 Client ID 仍会把授权悄悄导向上游，因此本版将默认 Client ID / 代理 URL 一并清空，做到凭据 100% 由你自己掌控。

#### 方案 A：用你自己的 Notion Public Integration（推荐，直连 Notion 换 token，不经过任何代理）

1. 打开 [Notion Integrations](https://www.notion.so/my-integrations) → `New integration` → **类型选 Public**（只有公共集成才支持 OAuth 登录）。
2. 在 `OAuth Domain & URIs` 中，把 **OAuth Redirect URI** 添加为扩展的回调地址：
   - `https://<你的扩展ID>.chromiumapp.org/`（Chrome / Edge 通用，由 `chrome.identity.getRedirectURL()` 自动生成）。
   - 扩展 ID 在 `chrome://extensions/`（或 Edge 扩展页）本项目「详细信息」中查看。
3. 保存后复制 `Client ID` 与 `Client secret`（OAuth client secret）。
4. 扩展控制面板「账号」标签页 → 高级设置「自定义 OAuth 应用（可选）」→ 填入你自己的 **Client ID + Client Secret** → 保存。
5. 点击「登录账号」：扩展用你的凭据**直接**对 `api.notion.com/v1/oauth/token` 交换 `access_token`，**全程不经过任何第三方代理**。

> **关键 caveat（务必阅读）**：Notion OAuth 返回的 `access_token` 与 weread2notion CLI 使用的 `ntn_` / `secret_` Internal Token 是**两套体系**；OAuth 复制出的页面通常也只有「模板」一个数据库，**并非** weread2notion 所需的「标准模板」（缺书架 / 日 / 周 / 月 / 年 / 分类 / 作者 7 个数据库）。因此授权登录后，仍需按上方「开始使用」四步法手动配置 `NOTION_TOKEN`（标准模板 Integration 的 token）与 `NOTION_PAGE`（标准模板页 URL）。OAuth 登录仅作为**便捷登录 / 复制公共模板入口**，不会覆盖你手动配置的标准同步目标。自动复制模板仅在你的 Public Integration 于 Notion 后台 `Configuration` 配置了「模板页 URL」且授权时勾选「复制模板」时生效。

#### 方案 B：GitHub 绑定（用于 GitHub Actions 自托管同步触发）

如果你用面板「账号」里的 GitHub 绑定，向**你自己 fork** 的仓库发 `workflow_dispatch` 触发同步：

1. GitHub → `Settings → Developer settings → OAuth Apps → New OAuth App`。
2. `Authorization callback URL` 填扩展回调：`https://<你的扩展ID>.chromiumapp.org/`。
3. 复制 `Client ID` 与 `Client secret`。
4. 控制面板「账号」标签页高级设置里填入你自己的 **GitHub Client ID + Secret** → 保存，再点「登录 GitHub」。

未配置时，GitHub 绑定按钮同样会提示「未配置 Client ID」。

**想保留「零配置」体验？** 那就不要 fork 本独立版，直接使用上游 `hodgekou/notionhub`（其内置了公共 OAuth 应用与 token-exchange 代理）。本仓库的定位是「代码完全自托管、OAuth 凭据 100% 由你自己掌控」。
