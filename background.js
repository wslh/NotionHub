...
/* Centralized NotionHub OAuth config.
 * Users click "登录账号" and authorize a Notion integration; Notion then
 * automatically duplicates the developer template into their workspace.
 *
 * Client ID resolution order:
 *   1. The value the user saved in the panel's advanced settings
 *      (chrome.storage.local "notion_oauth_client_id"). This lets the user
 *      override the built-in integration with their own Notion public
 *      integration if they wish.
 *   2. NOTIONHUB_DEFAULT_CLIENT_ID below — the production NotionHub public
 *      integration client_id, baked in so end users need not fill any field.
 *      Clicking "登录账号" performs the full OAuth flow automatically and the
 *      backend (NOTIONHUB_TOKEN_EXCHANGE_URL) exchanges the code for a token
 *      and duplicates the template.
 *
 * Token exchange: if the user also supplied a client_secret, the extension
 * exchanges the code directly with Notion; otherwise it proxies through
 * NOTIONHUB_TOKEN_EXCHANGE_URL (which holds the secret server-side). */
const NOTIONHUB_DEFAULT_CLIENT_ID = "3d7d872b-594c-817e-98ae-0037a0763231";
// SECURITY NOTE: the client_secret is bundled into the extension and therefore
// publicly extractable (anyone can unpack the extension and read it). This is
// acceptable only because it belongs to a dedicated NotionHub OAuth client used
// solely for auto-authorization. If it ever leaks, rotate it in the Notion
// integration settings. Do NOT reuse a secret tied to other services.
const NOTIONHUB_DEFAULT_CLIENT_SECRET = "";
const NOTIONHUB_TOKEN_EXCHANGE_URL = "https://notion-auth.notionhub.app/v1/oauth/exchange";

/* GitHub OAuth: same zero-config model as NotionHub.
 * These are tied to the fixed extension ID (manifest key). */
const GITHUBHUB_DEFAULT_CLIENT_ID = "Ov23li0qBa0u86zkw74E";
...
function isPlaceholderClient(id) {
  // The built-in NOTIONHUB_DEFAULT_CLIENT_ID already provides a valid client
  // id, so the only "missing" case is an empty/blank value.
  return !id || !id.trim();
}
...
/* Centralized NotionHub exchange: the extension does not store the
 * client_secret; the backend performs the Notion token exchange and returns
 * the access_token plus duplicated_template_id (when template duplication is
